...
export function Agent(config: AgentConfig): PropertyDecorator {
  return (target: object, propertyKey: string | symbol) => {
    const existing =
      (Reflect.getOwnMetadata(AGENT_METADATA_KEY, (target as any).constructor) as AgentMeta[] | undefined) ?? [];
    const updated: AgentMeta[] = [...existing, { propertyKey, config }];
    Reflect.defineMetadata(AGENT_METADATA_KEY, updated, (target as any).constructor);
  };
}
